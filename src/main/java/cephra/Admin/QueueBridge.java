...
public final class QueueBridge {
    ...
    private static void loadQueueFromDatabase() {
        try {
            ...
            List<Object[]> dbTickets = cephra.db.CephraDB.getAllQueueTickets();
    ...
    public static void addTicket(String ticket, String customer, String service, String status, String payment, String action) {
        ...
        int userBatteryLevel = cephra.db.CephraDB.getUserBatteryLevel(customer);
        ...
        cephra.db.CephraDB.setActiveTicket(customer, ticket, service, userBatteryLevel, "");
        
        // Add to database for persistent storage
        boolean dbSuccess = cephra.db.CephraDB.addQueueTicket(ticket, customer, service, status, payment, userBatteryLevel);
        ...
        if (!dbSuccess) {
            ...
            cephra.db.CephraDB.clearActiveTicket(customer);
        }
    ...
    private static void markPaymentPaidWithMethod(final String ticket, final String paymentMethod) {
        ...
        if (foundInRecords) {
            try {
                ...
                if (cephra.db.CephraDB.isPaymentAlreadyProcessed(ticket)) {
                ...
                boolean dbSuccess = cephra.db.CephraDB.processPaymentTransaction(
    ...
    private static void loadSettingsFromDatabase() {
        try {
            ...
            if (minFeeStr != null && !minFeeStr.trim().isEmpty()) {
                ...
                cephra.db.CephraDB.updateSystemSetting("minimum_fee", String.valueOf(MINIMUM_FEE));
            ...
            if (rateStr != null && !rateStr.trim().isEmpty()) {
                ...
                // Set default if not found in database
                cephra.db.CephraDB.updateSystemSetting("rate_per_kwh", String.valueOf(RATE_PER_KWH));
            ...
            if (multiplierStr != null && !multiplierStr.trim().isEmpty()) {
                ...
                // Set default if not found in database
                cephra.db.CephraDB.updateSystemSetting("fast_multiplier", String.valueOf(FAST_MULTIPLIER));
    ...
    public static void removeTicket(final String ticket) {
        ...
        if (!isAlreadyProcessed) {
            try {
                ...
                boolean dbRemoved = cephra.db.CephraDB.removeQueueTicket(ticket);
                if (!dbRemoved) {
    ...
    private static boolean isTicketInHistory(String ticket) {
        try {
            ...
            return cephra.db.CephraDB.isTicketInChargingHistory(ticket);
        } catch (Exception e) {
